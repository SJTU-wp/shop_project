...
class UserRegSerializer(serializers.ModelSerializer):
    ...
    def validate_code(self, code):
        # 不用get用filter是因为写起来比较麻烦，而且时间判断也麻烦
        # try:
        ...
        #     pass
        verify_records = VerifyCode.objects.filter(mobile=self.initial_data["username"]).order_by("-add_time")  # 以时间进行降序
        if verify_records:
